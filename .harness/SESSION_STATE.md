...
**Fecha de última actualización:** 2026-05-26 (Observabilidad: ProcessAudit, correlation ID, catálogo `op`, instrumentación P0)
...
- **2026-05-27 (Portfolio revalidación por edición):** Se actualiza harness de portfolio para explicitar regla de negocio: si item está `HIDDEN_PENDING_REVIEW` por `FLAGGED` y el profesional edita texto/fotos, se re-encola moderación IA (`RE_MODERATION`); si sale `OK`, auto-publica (`AUTO_RESTORE_AFTER_CORRECTION`). También se incorpora requisito de UX con mensaje claro de contenido no permitido.
- **2026-05-27 (Realtime moderación portfolio):** Se agrega spec/eval de `RealtimeModule` (Socket.IO server→client) para notificar cambios de estado de moderación sin polling agresivo. Contrato interno `EventEmitter2 → Gateway` (`realtime.push`) y evento `portfolio.moderation.completed` emitido por el worker `portfolio-moderate` hacia el room `user:<sub>` del profesional. Incluye Redis adapter para multi-instancia y fallback HTTP (invalidate + refetch).
- **2026-05-27 (Auditoría transversal P0/P1/P2):** Se inicia Oleada 0 de hardening global con artefactos `platform-resilience-audit.md` y `platform-resilience-audit-eval.md`. Objetivo: inventario por módulo de riesgos de transacción, idempotencia, fallos parciales y performance; priorización por severidad (P0 dinero/contrato, P1 performance crítica, P2 transversal) antes de implementar remediaciones.
- **2026-05-27 (Oleada 1 P0 — `approveCompletion` atómico):** `POST …/approve-completion` cierra el job y libera escrow en una sola transacción Prisma (`JobsRepository.approveCompletionAtomically` + `EscrowRepository.release` en TX). Cancelación de silent-acceptance y payout gateway quedan fuera de la TX (mismo patrón que `processSilentAcceptance`). Idempotente si el job ya está `CLOSED`.
- **2026-05-27 (Oleada 1 P0 — guards escrow):** `EscrowRepository.fundEscrow` y `release` usan `updateMany` con filtro de estado esperado (`PENDING`→`HELD`, `HELD`→`RELEASED`). Reintentos concurrentes son idempotentes si el escrow ya está en el estado destino (sin duplicar audit).
- **2026-05-27 (Oleada 1 P0 — idempotencia webhooks pagos):** Tabla `PaymentWebhookIdempotency` + `PaymentWebhookIdempotencyRepository`. MP y mock deduplican notificaciones; procesamiento concurrente → 503; fallos transitorios hacen `abandon` para reintento MP.
- **2026-05-27 (Oleada 1 P0 — billing subscribe/cancel/webhooks):** Subscribe persiste en TX y compensa `cancelPreapproval` si falla BD; cancel tolera MP ya cancelado y 503 si falla BD post-MP; webhooks billing usan `PaymentWebhookIdempotency` (`billing:mp:notify:…`); fetch pago MP fallido → 503.
- **2026-05-27 (Oleada 1 P0 — payout crash recovery):** Worker cron `escrow-payout-recovery` (`PayoutRecoveryBootstrap`/`PayoutRecoveryProcessor`) reintenta en modo gateway: (a) escrows `RELEASED + payoutStatus=PENDING` sin intentos previos y (b) intentos `PayoutAttempt.PENDING` “colgados” (older-than `PAYOUT_STUCK_ATTEMPT_MS`). `issuePayout` soporta `idempotencyKey` (`payout:{escrowId}:attempt:{n}`) para evitar dobles en retries. Configurable con `PAYOUT_RECOVERY_CRON`, `PAYOUT_RECOVERY_BATCH_SIZE`, `PAYOUT_STUCK_ATTEMPT_MS`.
- **2026-05-27 (Oleada 1 P0 — reconciliación payout por idempotencyKey):** `IPaymentGateway.reconcilePayoutByIdempotencyKey()`; recovery de intentos `PENDING` reconcilia antes de reemitir `issuePayout`. Provider `mercadopago`: consulta read-only MP (`payments/search` por `external_reference` + `payments/{id}`); sin token → mock. `issuePayout` MP sigue en mock hasta API payout MLU.
- **2026-05-27 (Moderación IA — hardening de formato y credenciales):** Se corrige integración con Rekognition: `ImagePrepService` pasa a salida JPEG (antes WebP) para evitar `Request has invalid image format`; `ai.config.ts` soporta `AI_AWS_ACCESS_KEY_ID` / `AI_AWS_SECRET_ACCESS_KEY` / `AI_AWS_SESSION_TOKEN` con fallback `AWS_*`. Se alinean docs (`.env.example`, tutorial y specs de portfolio/storage) al runtime actual.
...
- **2026-05-22 (Auditoría endpoints RBAC):** Matriz [api-endpoints-security.md](specs/api-endpoints-security.md). Código: `RolesGuard` en professional-profile, presign KYC, service-areas pro; `POST /users/company` onboarding `CLIENT` → `COMPANY_ADMIN`.
