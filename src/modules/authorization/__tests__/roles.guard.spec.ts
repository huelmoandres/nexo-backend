...
describe('RolesGuard', () => {
  const makeContext = (user: { sub?: string } = { sub: 'supabase-uid-1' }) =>
    ({
      getHandler: () => ({}),
      getClass: () => ({}),
      switchToHttp: () => ({
  ...
  it('permite cuando roles requeridos es array vacío', async () => {
    const reflector = {
  ...
  it('permite cuando el rol coincide', async () => {
    ...
    await expect(guard.canActivate(makeContext())).resolves.toBe(true);
    expect(authorizationService.assertUserHasAnyRole).toHaveBeenCalledWith(
      'supabase-uid-1',
      [Role.COMPANY_ADMIN],
    );
  });
